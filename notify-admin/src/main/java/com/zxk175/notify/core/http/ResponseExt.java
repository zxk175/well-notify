...
public class ResponseExt<T, EX> implements Serializable {
    ...
    private static <T extends Collection<?>, EX extends PageBeanVo> ResponseExt<T, EX> putPageExtra(T data, Long total, PageParam param, EX extra) {
        if (CollUtil.isEmpty(data)) {
            ResponseExt<T, EX> success = ResponseExt.success(ResponseMsg.NOT_FOUND_DATA);
            success.setData(data);

            extra.setHasPre(false);
            extra.setHasNext(false);
            extra.setPage(0L);
            extra.setSize(0L);
            extra.setTotal(0L);
            extra.setTotalPage(0L);
            success.setExtra(extra);
